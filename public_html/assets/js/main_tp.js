...
function startAjaxPnl() {
	...
	$.ajax({
        ...
        success: function (data) {
            if (!data || !data.orders || !data.assets) {
            ...
            data.assets.forEach(asset => {
                const bidPriceElement = $('.bid_price[data-asset-id="' + asset.id + '"]');
                const askPriceElement = $('.ask_price[data-asset-id="' + asset.id + '"]');
    
                if (bidPriceElement.length) {
                    bidPriceElement.text(asset.bid_price).removeClass('text-danger text-success');
                ...
                if (askPriceElement.length) {
                    askPriceElement.text(asset.ask_price).removeClass('text-danger text-success');
            ...
            $('.currentPL').text('$ ' + parseFloat(data.pnl).toFixed(2))
                .removeClass('text-danger text-success')
