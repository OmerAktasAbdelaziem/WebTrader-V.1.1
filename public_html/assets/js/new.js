$(function () {
    ...
    document.querySelectorAll(".generate-password").forEach(function (button) {
        button.addEventListener("click", function () {
            ...
            var closestPasswordInput =
                this.closest(".input-group").querySelector(".password");
    ...
    $(document).on("change", ".check-all-table", function () {
        ...
        var selectedValues = $("." + targetClass + ":checked")
            .map(function () {
                return $(this).val();
            })
            .get();
    ...
    $(document).on("change", ".check-number", function () {
        let checkedCount = $(".check-number:checked").length;
        var selectedValues = $(".check-number:checked")
            .map(function () {
                return $(this).val();
            })
            .get();
    ...
    $(document).on("click", ".multi-edit-btn", function () {
        let checkedValues = $(
            'input[type="checkbox"][form="addemployee"][name="clientid[]"]:checked'
        )
            .map(function () {
                return this.value;
            })
    ...
    if ($("#from-date-actions").length) {
        ...
        table
            .buttons()
            .container()
            .appendTo("#all-actions_wrapper .col-md-6:eq(0)");
    ...
    if ($(".filterable").length) {
        $(
            "#filter_name, #filter_email, #filter_phone, #filter_country, #filter_status"
        ...
        function filterTable() {
            ...
            nameFilter =
                nameFilter && nameFilter !== "" ? nameFilter.toLowerCase() : "";
            emailFilter =
            ...
            phoneFilter =
                phoneFilter && phoneFilter !== ""
                    ? phoneFilter.toLowerCase()
                    : "";
            countryTextFilter =
                countryTextFilter.length > 0 ? countryTextFilter : [];
            statusTextFilter =
                statusTextFilter.length > 0 ? statusTextFilter : [];
            ...
            $("tbody tr").each(function () {
                ...
                const email = $(this).data("email")
                    ? $(this).data("email").toLowerCase()
                    : "";
                const phone = $(this).data("phone")
                    ? $(this).data("phone").toLowerCase()
                    : "";
                const country = $(this).data("country")
                    ? $(this).data("country").toLowerCase()
                    : "";
                const status = $(this).data("status")
                    ? $(this).data("status").toLowerCase()
                    : "";

                const isNameMatch = nameFilter
                    ? name.includes(nameFilter)
                    : true;
                const isEmailMatch = emailFilter
                    ? email.includes(emailFilter)
                    : true;
                const isPhoneMatch = phoneFilter
                    ? phone.includes(phoneFilter)
                    : true;
                const isCountryMatch =
                    countryTextFilter.length > 0
                        ? countryTextFilter.some((s) => country.includes(s))
                        : true;
                const isStatusMatch =
                    statusTextFilter.length > 0
                        ? statusTextFilter.some((s) => status.includes(s))
                        : true; // Filter processing
