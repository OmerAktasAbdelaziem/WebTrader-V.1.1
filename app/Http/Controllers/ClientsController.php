...
class ClientsController extends Controller
{
    ...
    public function resetPassword(Request $request)
    {
        ...
        return redirect()->route('client.dashboard')->with('success', __('web.password_successfully_updated'));
    }
    ...
    public function processDeposit(Request $request)
    {
        ...
        $depositData = [
            ...
            'status'       => 'pending',
            'type'         => 'deposit',
            'usdt'         => Auth::guard('client')->user()->usdt,
            'bank_details' => null,
        ];
        MoneyTrx::create($depositData);

        return redirect()->back()->with('success', __('deposit_request_submitted_successfully'));
    }
...
//            }
    ...
    public function showQuotes(Request $request)
    {
        ...
        if ($asset_group_id) {
            ...
            $indicesAssets    = Asset::where('bid_price','!=',0)->where('category', 'Indx')->whereIn('id',$assetGroup->asset_ids)->whereNull('created_at')->get();
            $commodityAssets  = Asset::where('bid_price','!=',0)->where('category', 'Commodity')->whereIn('id',$assetGroup->asset_ids)->get();
            $favourite_assets = Asset::whereIn('id', $assetGroup->asset_ids)->whereIn('id', $favourite_assets_ids)->where('bid_price','!=',0)->whereIn('category', ['Crypto','Forex', 'Stocks','Indx'])->get();
        }
    ...
    public function showOrders(Request $request)
    {
        ...
        $finance       = $this->get_financial_data($user->broker_id,$session);
        switch ($type_filter) {
            case 'general':
                break;
    ...
    public function get_pnl($client_id, $asset_id = null, $from = null)
    {
        if ($asset_id) {
